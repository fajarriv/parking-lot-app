class ParkingManagementService
  ...
  def self.calculate_fee_in_cents(duration_hours, slot_type)
    ...
    hourly_fee = calculate_hourly_fee(total_hours % DAILY_HOURS, slot_type)

    daily_fee + hourly_fee
  end
  ...
  def self.calculate_hourly_fee(remainder_hours, slot_type)
    return 0 if remainder_hours == 0
    return FLAT_RATE_CENTS if remainder_hours <= FLAT_RATE_HOURS

    exceeding_hours = remainder_hours - FLAT_RATE_HOURS
